...
# 相比train19，删掉交叉注意力，无gated_fusion
...
class DualInputModel(nn.Module):
    ...
    def forward(self, geo_input, face_input):
        ...
        geo_global = torch.mean(geo_lstm, dim=1)  # (B, 128)
        ...
        # ----------------------------
        # ✅ 使用 GatedFusion 融合两个全局特征
        # ----------------------------
        #final = self.gated_fusion(geo_global, img_global)  # (B, 128)
        final = geo_global + img_global  # (B, D)
        # ----------------------------
        # 分类输出
        # ----------------------------
