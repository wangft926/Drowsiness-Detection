...
def is_fatigue(model_window_outputs, fps):
    ...
    # Rule 1: 眨眼频率检测
    # 连续3帧闭眼(类别0)判定为一次眨眼，一分钟内眨眼超过25次判定为疲劳
    blink_count = 0
    i = 0
    while i < len(frame_predictions):
        if frame_predictions[i] == 0:  # 闭眼为0
            # 检查是否连续3帧闭眼
            consecutive_count = 0
            j = i
            while j < len(frame_predictions) and frame_predictions[j] == 0:
                consecutive_count += 1
                j += 1
    ...
    if len(frame_predictions) > 0:
        blink_rate_per_minute = blink_count * frames_per_minute / len(frame_predictions)
        if blink_rate_per_minute > 25:
            print(f"Blink rate: {blink_rate_per_minute:.2f} times per minute")
            return "Yawning", 1

    # Rule 2: 打哈欠频率检测
    # 连续3帧打哈欠(类别2)判定为一次打哈欠，一分钟内打哈欠超过5次判定为疲劳
    yawn_count = 0
    i = 0
    while i < len(frame_predictions):
        if frame_predictions[i] == 2:  # 打哈欠为2
            ...
            consecutive_count = 0
            j = i
            while j < len(frame_predictions) and frame_predictions[j] == 2:
                consecutive_count += 1
                j += 1

            # 如果连续打哈欠达到了3帧，则计为一次打哈欠
            if consecutive_count >= 3:
                yawn_count += 1
                # 跳过这段连续的打哈欠帧，避免重复计算
                i = j
    ...
    # 计算等效的一分钟打哈欠次数
    if len(frame_predictions) > 0:
        yawn_rate_per_minute = yawn_count * frames_per_minute / len(frame_predictions)
        if yawn_rate_per_minute > 5:
            print(f"Yawn rate: {yawn_rate_per_minute:.2f} times per minute")
            return "Yawning", 2
