...
def calculate_mar(landmarks, mouth_indices):
    A = np.linalg.norm(landmarks[mouth_indices[2]] - landmarks[mouth_indices[3]])
    B = np.linalg.norm(landmarks[mouth_indices[0]] - landmarks[mouth_indices[7]])
    mar = A / B
    return mar
...
def calculate_head_pose(frame, landmarks):
    ...
    image_points = np.float32([
        [landmarks[0][0] * w, landmarks[0][1] * h],  # Nose tip
        [landmarks[1][0] * w, landmarks[1][1] * h],  # Chin
        [landmarks[2][0] * w, landmarks[2][1] * h],  # Left eye left corner
        [landmarks[3][0] * w, landmarks[3][1] * h],  # Right eye right corner
        [landmarks[4][0] * w, landmarks[4][1] * h],  # Left Mouth corner
        [landmarks[5][0] * w, landmarks[5][1] * h],  # Right mouth corner
    ])
    ...
    phi = rotation_matrix[0, 2]
    theta = rotation_matrix[1, 2]
    return phi, theta
...
def predict_video(video_path, detector, model, window_size=16, overlap=8, norm_file='norm.npy'):
    ...
    print("All features[0] (R_EAR):", all_features[0].min(), all_features[0].max())
    print("All features[2] (MAR):", all_features[2].min(), all_features[2].max())
    # 加载训练阶段保存的 min/max 参数
    norm_params = np.load(norm_file)  # shape: (5, 2)
    # 打印 norm 文件内容
    print("Norm params shape:", norm_params.shape)
    print("Norm params:\n", norm_params)
    # 对每个特征通道进行统一归一化（⚠️ 关键修改点）
    for i in range(5):
    ...
    with torch.no_grad():
        for i in range(len(windows)):
            ...
            out = model(geo, face)
            pred_class = int(out.argmax(dim=1).cpu().item())
            outputs.append(pred_class)

    # 打印调试信息
    print("Window feature shape:", windows.shape)  # 应该是 (N, 5, 16)
    print("Face window shape:", face_windows.shape)  # 应该是 (N, 16, H, W, C)
    print("Sample logits:", out.cpu().numpy())  # 查看三类得分

    return outputs, frame_count
