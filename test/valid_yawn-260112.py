...
def main():
    ...
    for idx, (root, file) in enumerate(all_files):
        ...
        # 根据文件名，读取对应的csv，是每帧的标签文件，比如G:\A_wangft_bs\Drowsiness-Detection-using-CNN-and-LSTM-main\YawDD非36个的外部验证数据集_label-bak\9-MaleNoGlasses-Normal.csv
        label_dir = r"../YawDD非36个的外部验证数据集_label"
    ...
    file_pbar.close()
    # 计算平均准确率
    total_correct = sum(result['correct_predictions'] for result in results)
    ...
    average_accuracy = sum(result['accuracy'] for result in results) / len(results) if results else 0

    print(f"总体准确率（总正确预测帧数/总帧数）: {overall_accuracy:.4f}")
    print(f"平均准确率（各视频准确率平均值）: {average_accuracy:.4f}")

    # 添加平均准确率到DataFrame
    avg_row = {
        'video_name': 'Overall_Average',
