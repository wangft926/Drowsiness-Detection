...
# wft实验全集，相比train0_3.py，不冻结mobilenet_v3_small，删掉交叉注意力，无gated_fusion
# 只有图像特征，加几何特征，进行融合，但是无GRU，无BiGRU
...
class DualInputModel(nn.Module):
    def __init__(self, config):
        ...
        # ----------------------------
        self.cnn_img = FrameFeatureExtractor()  # 冻结

        # ----------------------------
        # 几何特征分支：简单线性层处理
        # ----------------------------
        self.geo_conv = nn.Sequential(
            nn.Conv1d(in_channels=5, out_channels=64, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.AdaptiveAvgPool1d(1)
        )
        self.geo_linear = nn.Sequential(
            nn.Linear(64, 128),
            nn.BatchNorm1d(128),
            nn.ReLU(),
            nn.Dropout(0.5)
        ...
        # ✅ 融合两个全局特征（这里使用简单的相加）
        # ----------------------------
        # 如果需要更复杂的融合方法，如 GatedFusion，也可以在此处添加
    ...
    def forward(self, geo_input, face_input):
        ...
        img_global = torch.mean(face_out, dim=1)  # (B, 128)

        # ----------------------------
        # 几何特征处理：Conv1D 提取时空特征
        # ----------------------------
        geo_features = self.geo_conv(geo_input)  # (B, 64, 10) → Adaptive Pooling → (B, 64, 1)
        geo_global = geo_features.squeeze(-1)  # (B, 64)
        geo_global = self.geo_linear(geo_global)  # (B, 128)
        # ----------------------------
        # ✅ 融合两个全局特征（简单相加）
        # ----------------------------
        final = img_global + geo_global  # (B, 128)
