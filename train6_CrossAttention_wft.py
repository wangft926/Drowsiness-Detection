...
class DualInputModel(nn.Module):
    ...
    def forward(self, geo_input, face_input):
        ...
        # Cross Attention: Geo 查询 Image
        attn_output, _ = self.cross_attn(
            query=geo_out.transpose(0, 1),
            key=face_out.transpose(0, 1),
            value=face_out.transpose(0, 1)
        )
        ...
        # 融合最后时刻的 LSTM 输出和 attention 输出
        final = combined.unsqueeze(1) + lstm_out[:, -1:, :]  # (B, 1, 128)

        out = self.classifier(final.squeeze(1))  # (B, num_classes)
        return out
