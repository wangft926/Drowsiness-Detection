...
class DualInputModel(nn.Module):
    def __init__(self, config):
        ...
        self.gru_geo = nn.GRU(64, 128, batch_first=True, bidirectional=True)
        ...
        # 几何 → 图像 注意力
        self.cross_attn_geo_to_img = nn.MultiheadAttention(embed_dim=64, num_heads=2)

        # 图像 → 几何 注意力
        self.cross_attn_img_to_geo = nn.MultiheadAttention(embed_dim=64, num_heads=2)
    ...
    def forward(self, geo_input, face_input):
        ...
        gru_geo_out, _ = self.gru_geo(geo_out)  # (B, T, 256)
        geo_lstm = self.proj_geo(gru_geo_out)  # (B, T, 128)
        # geo_final = geo_lstm.mean(dim=1)  # (B, 128)
        ...
        # img_final = img_lstm.mean(dim=1)  # (B, 128)
