...
# 用MobileNet，无# Bottleneck 层 + BN

class FrameFeatureExtractor(nn.Module):
    def __init__(self, freeze_backbone=True):  # 新增参数
        super().__init__()
...
class DualInputModel(nn.Module):
    def __init__(self, config):
        ...
        # ----------------------------
        self.cnn_img = FrameFeatureExtractor(freeze_backbone=True)  # 冻结
        # # 图像帧分支（3D CNN）
        ...
        #     nn.BatchNorm1d(128)
        # )
        #self.feature_proj = nn.Linear(32 * 32 * 32, 128)
        # 图像 LSTM 分支
        self.gru_img = nn.GRU(
            input_size=128,
            hidden_size=64,  # 降低隐藏层大小
            bidirectional=True,
            batch_first=True,
            dropout=0.5
        )
        self.norm_img = nn.LayerNorm(128)  # GRU 输出是 (B, T, 256)

        # Linear 投影
        self.proj_img = nn.Linear(128, 128)
    ...
    def forward(self, geo_input, face_input):
        ...
        # 使用 projection 层降维到 128
        #face_out = self.feature_proj(face_out)  # (B, T, 128)
        face_out, _ = self.gru_img(face_out)  # (B, T, 64)
        face_out = self.norm_img(face_out)  # ✅ LayerNorm on 256-dim
        face_out = self.proj_img(face_out)  # (B, T, 128)
        # 取最后一帧作为最终表示
        combined = face_out[:, -1, :]  # (B, 64)
