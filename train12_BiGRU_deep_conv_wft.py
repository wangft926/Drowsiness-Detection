...
class DualInputModel(nn.Module):
    def __init__(self, config):
        ...
        # )
        self.cnn_geo = nn.Sequential(
            depthwise_separable_conv1d(config['num_features'], 32, kernel_size=3, padding=1),
            nn.BatchNorm1d(32),
            nn.ReLU(),
            depthwise_separable_conv1d(32, 64, kernel_size=3, padding=2),
            nn.BatchNorm1d(64),
            nn.ReLU(),
            nn.Dropout(0.3)
        )
        # self.lstm = nn.GRU(64, 128, batch_first=True)
        # 此时输出维度会变成 (B, T, 256)，你需要调整后续投影层：
        self.lstm = nn.GRU(64, 128, batch_first=True, bidirectional=True)
        # 将 BiGRU 输出压缩回 128 维，额外的线性层 proj_bilstm 来将 BiGRU 的输出从 256 维压缩回 128 维，以便与 Cross Attention 输出进行一致的维度匹配。
        self.proj_bilstm = nn.Linear(256, 128)
        # 图像帧分支
        # self.cnn_img = nn.Sequential(
        #     nn.Conv3d(3, 16, kernel_size=(3, 3, 3), padding=(1, 1, 1)),
        #     nn.BatchNorm3d(16),  # 新增
        #     nn.ReLU(),
        #     nn.MaxPool3d((1, 2, 2)),
        #     nn.Conv3d(16, 32, kernel_size=(3, 3, 3), padding=(1, 1, 1)),
        #     nn.BatchNorm3d(32),  # 新增
        #     nn.ReLU(),
        #     nn.MaxPool3d((1, 2, 2))
        # )
        self.cnn_img = nn.Sequential(
            depthwise_separable_conv3d(3, 16, kernel_size=3, padding=1),
            nn.BatchNorm3d(16),
            nn.ReLU(),
            nn.MaxPool3d((1, 2, 2)),
            depthwise_separable_conv3d(16, 32, kernel_size=3, padding=1),
            nn.BatchNorm3d(32),
            nn.ReLU(),
            nn.MaxPool3d((1, 2, 2))
        )
        ...
        self.cross_attn = nn.MultiheadAttention(embed_dim=64, num_heads=2)

        # Query 向量（可学习）
        self.learnable_query = nn.Parameter(torch.randn(1, 1, 64))  # (1, 1, 64)
        ...
        self.classifier = nn.Sequential(
            ...
            nn.ReLU(),
            nn.BatchNorm1d(64),
            nn.Dropout(0.5),  # 新增 dropout 防止过拟合
    ...
    def forward(self, geo_input, face_input):
        ...
        # 不要只取最后一个时间步，尽量用 mean pooling；如果是 BiGRU/BiLSTM，就先用 Linear 压缩维度，再做 mean pooling。
        geo_final = self.proj_bilstm(lstm_out).mean(dim=1)
        ...
        # 使用可学习的 query 向量做 Cross Attention
        learnable_queries = self.learnable_query.expand(-1, B, -1)  # (1, B, 64)
        geo_query = self.attn_norm(geo_out.transpose(0, 1))  # (T, B, 64)

        # 合并可学习 query 和 geo_query
        queries = torch.cat([learnable_queries, geo_query], dim=0)  # (T+1, B, 64)

        # Cross Attention: query from geo & learnable, key/value from image
        attn_output, _ = self.cross_attn(
            query=queries,
            key=self.attn_norm(face_out.transpose(0, 1)),
            value=self.attn_norm(face_out.transpose(0, 1))
        )

        # 取前几个位置输出（如只取 learnable query 输出）
